...
# It is very important to initialize the server globally and outside of main
#  so the MCP client can find and connect to it
...
def main():
    ...
    logger.info(f"Starting MCP server on localhost:{port}")
    logger.info("Server will be accessible via Streamable HTTP transport")

    try:
        # Use FastMCP's built-in run method with Streamable HTTP transport
        mcp.run(transport="streamable-http", host="localhost", port=port, path="/mcp")
